 ...
 *   pnpm strata check              # すべての注釈の形式・置き場所・暗号化・整合（後方参照のみ）を検査する（CI と pre-commit hook）
 *   pnpm strata decrypt <slug>     # strata/private/<slug>.json を復号して <slug>.plain.json（.gitignore 対象）を作る
 *   pnpm strata encrypt <slug>     # <slug>.plain.json を暗号化して strata/private/<slug>.json に書き、平文を削除する
 *
 * 注意:
...
function usage(): never {
	console.error(
		"使い方: pnpm strata pending | catalog <slug> | text <slug> | check | decrypt <slug> | encrypt <slug>",
	);
	process.exit(2);
}
...
/**
 * すべての注釈を読み込む。decryptPrivate が true のときは限定記事の注釈を sops で復号して要約を平文にする。
 */
function loadAnnotations(
	decryptPrivate: boolean,
): Map<string, StrataAnnotation> {
	const annotations = new Map<string, StrataAnnotation>();
	for (const relativePath of listStrataFiles()) {
		...
		const annotation = parseStrataAnnotation(content, fileName);
		annotations.set(annotation.slug, annotation);
	}
	return annotations;
}
...
function encrypt(slug: string): void {
	...
	writeFileSync(
		...
		encryptJson(
			...
			path.join(
				STRATA_DIR,
				strataRelativePath(annotation.slug, post.visibility),
			),
		),
		"utf8",
	);
